...
interface App {
  ...
  xattr_python(): Promise<void>;
  save_config(config: Config): Promise<void>;
  open_websocket(port: number): Promise<void>;
  shutdown_websocket(): Promise<void>;
  is_websocket_alive(): Promise<boolean>;
  show_windows_from_capture(windows: WindowLabel[]): Promise<void>;
  hide_windows_from_capture(windows: WindowLabel[]): Promise<void>;
  path_exists(path: string): Promise<boolean>;
}

class KiwiApp implements App {
  name: string = "";
  version: string = "";
