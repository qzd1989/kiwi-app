...
impl Frame {
    pub fn recognize_text(&self, start_point: Point, size: Size) -> Result<(Option<String>, u128)> {
        let now = Instant::now();
        ...
        if texts.is_empty() {
            return Ok((None, now.elapsed().as_millis()));
        }
        Ok((Some(texts), now.elapsed().as_millis()))
    }
}
//只想让它初始化一次
